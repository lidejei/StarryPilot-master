...
float adrc_fhan(float v1, float v2, float r0, float h0)//最速fhan函数
{
	float d = h0 * h0 * r0;
	float a0 = h0 * v2;
	float y = v1 + a0;
...
float adrc_fal(float e, float alpha, float delta)//fal函数
{
	if(fabsf(e) <= delta){
...
void adrc_td_init(ADRC_TD_Def* td_t, float h, float r0, float h0)//初始化微分跟踪器
{
	td_t->h = h;
	td_t->r0 = r0;
	td_t->h0 = h0;
...
void adrc_td(ADRC_TD_Def* td, float v)//微分跟踪器
{
...
void adrc_eso_init(ADRC_ESO_Def* eso_t, float h, float beta1, float beta2, float alpha, float delta, float b0)//eso状态观测器初始化
{
	eso_t->h = h;
	eso_t->beta1 = beta1;
	eso_t->beta2 = beta2;
